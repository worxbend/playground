...
import com.worxbend.persistence.maintenance.AdvisoryLock
import com.worxbend.persistence.maintenance.MonthPartition
import com.worxbend.persistence.maintenance.PartitionCalendar
import com.worxbend.persistence.maintenance.PartitionDdl
...
import com.worxbend.persistence.maintenance.RefreshReport
...
final class MaintenanceIT extends PostgresSuite:
  ...
  test("the remedy the job reports is real: running it moves the rows and attaches the month"):
    ...
    assertEquals(query(s"SELECT ce_type FROM ${month.qualifiedName}"), Vector("com.worxbend.it.maintenance"))
    truncateEvents()
  ...
  private def insertEvent(occurredAt: Instant, id: String): Unit =
    val raw =
      s"""{"specversion":"1.0","id":"$id","source":"urn:worxbend:it:maintenance",""" +
        """"type":"com.worxbend.it.maintenance","data":{"deviceId":"device-it","severity":"info"}}"""
