...
**Partition maintenance cannot live in Flyway** (migrations are versioned and immutable; partitions are a rolling concern). A Pekko-scheduled idempotent job creates months N+3 ahead with `CREATE TABLE IF NOT EXISTS … PARTITION OF`, and a `REFRESH MATERIALIZED VIEW CONCURRENTLY` job runs on its own schedule. Both are guarded by a `pg_try_advisory_lock` so multiple replicas do not race.

> **Amendment (as built).** This paragraph originally assigned both jobs to **ferrite**; they were implemented in **cobalt**, and that is the arrangement that shipped. The reason is the one this document gives elsewhere for cobalt owning the migrations: ferrite is a reader, its pools are opened with `read-only = true`, and giving the web tier a DDL path it otherwise has no use for would undo the separation §1 exists to draw. cobalt already holds the write pool and already runs Flyway, so the job that keeps the write path writable belongs beside them. The lock, the schedule and the meters are unchanged — see `modules/persistence/…/maintenance/` and cobalt's `MaintenanceJobs`.
...
| **Partition maintenance cannot live in Flyway.** Missing future partitions ⇒ `no partition of relation found for row` — the most common way a partitioned design breaks in production. | Advisory-locked scheduled job creating N+3 months ahead; a metric + alert on "months of headroom". Built in **cobalt**, not ferrite — see the amendment in §5. |
